...
@section('content')
    ...
    <div class="flex mb-5 gap-4 flex-wrap justify-between">
        <button type="button" onclick="tAd()"
        ...
        <label class="block text-xs">
            <form action="/cari-auditorium" method="GET">
    ...
    <a href="/tambah-auditorium" id="ttAd" hidden></a>
    <div class="w-full mb-4 overflow-hidden rounded-lg shadow-xs">
        ...
        <div
            ...
            <span class="flex items-center col-span-3">
                Showing {{ $dt_aud->firstItem() }} - {{ $dt_aud->lastItem() }} of {{ $dt_aud->total() }}
            </span>
            <span class="col-span-2"></span>
            <!-- Pagination -->
            {{ $dt_aud->links('layouts.pagination') }}
        </div>
    </div>
@endsection
...
@section('js')
    <script>
        function tAd() {
            document.getElementById('ttAd').click()
        }
    </script>
@endsection
